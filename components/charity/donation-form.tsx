...
export function DonationForm() {
  ...
  async function handleCreateDonation() {
    const amountMinor = parseInt(amount, 10) * 100;
    if (!selectedCharityId) { setError("Please select a charity"); return; }
    if (isNaN(amountMinor) || amountMinor < 100) { setError("Minimum donation is £1.00"); return; }
  ...
  return <Card>
    ...
    <CardContent className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2"><Label htmlFor="amount">Donation Amount (GBP)</Label><Input id="amount" type="number" min="1" max="10000" step="1" value={amount} onChange={(e) => setAmount(e.target.value)} disabled={isCreating} placeholder="10.00" /><p className="text-xs text-muted-foreground">Minimum £1.00, maximum £10,000.00</p></div>
