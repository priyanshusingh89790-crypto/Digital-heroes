...
import { simulateDraw as engineSimulateDraw, isValidStateTransition, canPublishDraw, canModifyDraw, validateDrawConfig, validatePrizeConfig, prepareAlgorithmicConfig } from "./draw-engine";
...
type SubscriptionRow = {
  user_id: string;
  unit_amount_minor: number | string | null;
...
async function getEligibleSubscriptions(admin: ReturnType<typeof createAdminClient>, drawMonth: string): Promise<SubscriptionRow[]> {
...
function subscriptionTotalAmount(subscriptions: SubscriptionRow[]) {
...
async function buildSimulation(draw: DrawRow, seed?: string): Promise<DrawSimulation> {
  const admin = createAdminClient();
  const subscriptions = await getEligibleSubscriptions(admin, draw.draw_month);
  const eligibleUserIds = [...new Set(subscriptions.map((subscription) => subscription.user_id))];
  if (eligibleUserIds.length === 0) {
    ...
    const prizeConfig: PrizePoolConfig = { currency: "GBP", contributionPercentage: 50, activeSubscriberCount: 1, subscriptionAmountMinor: 0, jackpotRolloverInMinor: 0 };
    return engineSimulateDraw(config, [], prizeConfig, seed);
  }
...
export async function simulateDraw(input: z.infer<typeof simulateDrawSchema>): Promise<DrawSimulation> {
  ...
  const simulation = await buildSimulation(draw as DrawRow, validated.seed);
  const now = new Date().toISOString();
  const { error } = await admin.from("draws").update({ status: "simulated", numbers: simulation.drawResult.numbers, simulated_at: now, generation_audit: { ...(draw.generation_audit as Record<string, unknown> | null), simulated_at: now, simulated_by: profile.id, simulation_result: { draw_numbers: simulation.drawResult.numbers, eligible_participants: simulation.eligibleParticipants, total_pool: simulation.prizePool.totalPoolMinor, winners_count: simulation.winners.length } } }).eq("id", validated.draw_id);
...
export async function publishDraw(input: z.infer<typeof publishDrawSchema>) {
  ...
  const { data: rawDraw, error: drawError } = await admin.from("draws").select("*").eq("id", validated.draw_id).single();
  if (drawError || !rawDraw) throw new Error("Draw not found");
  ...
  const now = new Date().toISOString();
  const { error: updateError } = await admin.from("draws").update({ status: "published", numbers: finalNumbers, published_at: now, algorithm_version: draw.type === "random" ? "random-v1" : "algorithmic-v1", generation_audit: { ...(draw.generation_audit as Record<string, unknown> | null), published_at: now, published_by: profile.id } }).eq("id", validated.draw_id);
...
export async function cancelDraw(drawId: string) { const { profile } = await requireAdmin(); const admin = createAdminClient(); const { data: draw, error: fetchError } = await admin.from("draws").select("*").eq("id", drawId).single(); if (fetchError || !draw) throw new Error("Draw not found"); if (!isValidStateTransition(draw.status as DrawStatus, "cancelled")) throw new Error(`Cannot cancel draw with status '${draw.status}'`); const { error: updateError } = await admin.from("draws").update({ status: "cancelled", generation_audit: { ...(draw.generation_audit as Record<string, unknown> | null), cancelled_at: new Date().toISOString(), cancelled_by: profile.id } }).eq("id", drawId); if (updateError) throw new Error(`Failed to cancel draw: ${updateError.message}`); return { success: true }; }
